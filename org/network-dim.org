...
* Introduction
  ...
  activity data depends on the connectivity of neurons. Spike-in data can be
  represented as a matrix \([x_{ij}]\), \(i = 1, \ldots, n\), \(j = 1, \ldots,
  p\) where \(x_{ij}\) denotes the number of times neuron \(i\) fired in time
  interval \([t_{j - 1}, t_{j})\).
