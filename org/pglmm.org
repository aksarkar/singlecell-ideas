...
** LMM
   ...
   i.e., the posterior of the random effects, given the fixed effects, the
   model hyperparameters, and the data, is analytic and is also Gaussian.
...
** Poisson GLMM
   ...
   Another possibility is to use variational inference. However, it is easy to
   ...
   \begin{equation}
     q^*(\vb) \propto \exp\left(\sum_i \left[y_i (\mx\vb)_i - y_{i+}\exp((\mx\vb + \mz\vu)_i)\right] - \frac{\vb'\vb}{2}\right).
   \end{equation}
   ...
   data are very informative about the mean).

   *Remark* One way to get around this limitation could be
...
** Variational Empirical Bayes
   ...
   \begin{equation}
     \ell = \E_{q(\vb, \vu)}\left[\left[\sum_i \ln p(y_i \mid y_{i+}, \mx, \mz, \vb, \vu)\right] - \KL(q(\vb) \Vert p(\vb)) - \KL(q(\vu) \Vert p(\vu)),
   \end{equation}
   ...
   the KL divergence is analytic (for the choice of conjugate variational
