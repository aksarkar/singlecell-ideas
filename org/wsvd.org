...
** Srebro and Jaakkola 2003
   ...
   \[ \min \sum_{i,j} W_{ij} \left(X_{ij} - [\mathbf{U} \mathbf{V}']_{ij} \right)^2 \]

   The fundamental iteration is:

   \[ [\mathbf{UV}']^{(t + 1)} = \mathrm{LRA}_k(\mathbf{W} \circ \mathbf{X} +
   (\mathbf{1} - \mathbf{W}) \circ [\mathbf{UV}']^{(t)}) \]

   #+BEGIN_SRC ipython
     def srebro_wsvd(x, w, n_components, max_iters=10, verbose=False):
       n, p = x.shape
       low_rank = x
       pca = skd.PCA(n_components=n_components)
       obj = float('inf')
