...
* Introduction
  ...
  variation of interest (Sarkar and Stephens 2021). The main disadvantage of
  strategy (2) is that it does not account for transcriptional noise
...
** Variational autoencoder
   ...
   This is a simplification of the original presentation of VAEs (Kingma and
   Welling 2014); in this for, it is a simple generalization of PPCA
   ...
   #+BEGIN_SRC ipython
     class VAE(torch.nn.Module):
       def __init__(self, input_dim, latent_dim, hidden_dim=128):
         super().__init__()
         # Roughly 1e-3
         # self.residual_scale_raw = torch.nn.Parameter(torch.tensor(-9.))
         self.prior_scale_raw = torch.nn.Parameter(torch.tensor(-9.))
         self.encoder = DeepGaussian(input_dim, latent_dim, hidden_dim)
         self.decoder = DeepGaussian(latent_dim, input_dim, hidden_dim)

       def forward(self, x, writer=None, global_step=None):
         softplus = torch.nn.functional.softplus
         z_mean, z_scale = self.encoder.forward(x)
         qz = torch.distributions.Normal(z_scale, z_scale)
         kl = torch.distributions.kl.kl_divergence(qz, torch.distributions.Normal(0., softplus(self.prior_scale_raw))).sum(dim=1)
         x_mean, x_scale = self.decoder.forward(qz.rsample())
         err = torch.distributions.Normal(x_mean, x_scale).log_prob(x).sum(dim=1)
         loss = -(err - kl).sum()
       ...
       def fit(self, data, n_epochs, log_dir=None, **kwargs):
       ...
       def predict(self, data):
         xhat = []
         for (x,) in data:
           xhat.append(self.decoder.forward(self.encoder.forward(x)[0])[0].cpu().numpy())
...
** Gaussian GMVAE
   ...
   \begin{align}
     \vx_i \mid \vz_i &\sim \N(m(\vz_i), \diag(s^2(\vz_i)))\\
     \vz_i \mid \vy_i &\sim \N(m_0(\vy_i), \diag(s_0^2(\vy_i)))\\
   ...
   - \(m(\cdot), s^2(\cdot)\) are fully-connected neural networks mapping
     latent variables to the noiseless observation and the observation noise,
     respectively
   - \(\vy_i\) is a one-hot \(k\)-vector denoting the latent cluster assignment
   - \(m_0(\cdot), s_0^2(\cdot)\) are fully-connected neural networks mapping
   ...
   [[https://openreview.net/forum?id=33X9fd2-9FyZd][Rui Shu]] with the
   likelihood of [[https://arxiv.org/abs/1611.02648][Dilokthanakul et
   al. 2016]]. Specifically, the (marginal) prior on latent variables \(\vz_i\)
   is a mixture of Gaussians. To perform inference, introduce a variational
   approximation
     ...
     class GMVAE(torch.nn.Module):
       ...
       def forward(self, x, labels=None, writer=None, global_step=None, eps=1e-16):
         ...
         for k in range(self.n_clusters):
           ...
           qz = torch.distributions.Normal(z_mean, z_scale).rsample()
           loss += (probs[:,k].reshape(-1, 1)
                    ,* (torch.distributions.Normal(z_mean, z_scale).log_prob(qz)
                       - torch.distributions.Normal(prior_mean[k], prior_scale[k]).log_prob(qz))).sum()
           assert not torch.isnan(loss)
           ...
           loss -= (probs[:,k].reshape(-1, 1)
                    ,* torch.distributions.Normal(
                      x_mean,
                      torch.nn.functional.softplus(self.residual_scale_raw)).log_prob(x)).sum()
           assert not torch.isnan(loss)
           assert loss > 0
         if writer is not None:
           writer.add_scalar('loss/neg_elbo', loss, global_step)
...
** Pinwheel example
   ...
   Use the "pinwheel data" from [[https://arxiv.org/abs/1603.06277][Johnson et
   al. 2016]]
   ([[https://github.com/mattjj/svae/blob/master/experiments/gmm_svae_synth.py#L11][source
   code]] associated with [[https://arxiv.org/abs/1611.02648][Dilokthanakul et
   al. 2016]]).
   ...
   Generate data following [[https://arxiv.org/abs/1611.02648][Dilokthanakul et
   al. 2016]].
...
*** VAE
   ...
   #+BEGIN_SRC ipython :async t
     ...
     fit0 = VAE(input_dim=2, latent_dim=latent_dim)
     fit0 = fit0.fit(data,
                  lr=lr,
                  n_epochs=n_epochs,
                  log_dir=f'runs/gmvae/pinwheel-vae-{latent_dim}-{seed}-{lr:.1g}-{batch_size}-{n_epochs}'
             )
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/zinb-vae-gmm.org/pinwheel-ex-vae-zhat-umap.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(3.5, 3)
     for k in range(K):
       plt.scatter(u[labels == k,0], u[labels == k,1], s=1, color=cm(k), label=f'C{k}')
     plt.legend(frameon=False, handletextpad=0, markerscale=4, loc='center left', bbox_to_anchor=(1, .5))
   ...
   :RESULTS:
   # Out[209]:
   [[file:figure/zinb-vae-gmm.org/pinwheel-ex-vae-zhat-umap.png]]
   :END:

   Report the estimated \(\hat{s}_0\).
   ...
   :RESULTS:
   # Out[210]:
   : array(0.0014072, dtype=float32)
   :END:

   Plot the predicted values.

   #+BEGIN_SRC ipython :ipyfile figure/zinb-vae-gmm.org/pinwheel-ex-vae-xhat.png
     data = td.DataLoader(
       td.TensorDataset(
         torch.tensor(dat, device='cuda', dtype=torch.float)),
       batch_size=batch_size)
     xhat = fit0.predict(data)
   ...
   :RESULTS:
   # Out[211]:
   [[file:figure/zinb-vae-gmm.org/pinwheel-ex-vae-xhat.png]]
   :END:

   Report the estimated \(\hat{s}\).
