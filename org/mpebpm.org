...
** Goodness of fit
   ...
   :RESULTS:
   # Out[75]:
   : (29, 0.01183673469387755)
   :END:

   Plot the simulated data, ground truth \(g\), and estimated \(\hat{g}\) for
   one of the simulation trials where the data significantly departed from the
   estimated distribution.

   #+BEGIN_SRC ipython
     kwargs = dict(sig.iloc[0][['num_samples', 'num_mols', 'log_mu', 'log_phi', 'logodds', 'trial']])
     kwargs['num_samples'] = int(kwargs['num_samples'])
     kwargs['size'] = kwargs.pop('num_mols')
     kwargs['seed'] = int(kwargs.pop('trial'))
     x = scqtl.simulation.simulate(**kwargs)[0][:,:1]     
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/mpebpm.org/sim-ex.png
     ...
     F = sp.expit(-query['logodds']) * st.nbinom(n=np.exp(-query['log_phi']), p=1 / (1 + 1e5 * np.exp(query['log_mu'] + query['log_phi']))).pmf(grid)
     F[0] += sp.expit(query['logodds'])
     Fhat = sp.expit(-query['logodds_hat']) * st.nbinom(n=np.exp(-query['log_phi_hat']), p=1 / (1 + 1e5 * np.exp(query['log_mu_hat'] + query['log_phi_hat']))).pmf(grid)
     Fhat[0] += sp.expit(query['logodds_hat'])
     ax[0].hist(x, bins=grid, color='k')
     ax[0].plot(grid + 0.5, 100 * F, c=cm(0))
     ax[0].plot(grid + 0.5, 100 * Fhat, c=cm(1))
     ax[0].set_xlabel('Number of molecules')
     ax[0].set_ylabel('Number of cells')

     grid = np.linspace(0, 1e-3, 1000)
     G = sp.expit(query['logodds']) + sp.expit(-query['logodds']) * st.gamma(a=np.exp(-query['log_phi']), scale=np.exp(query['log_mu'] + query['log_phi'])).cdf(grid)
     Ghat = sp.expit(query['logodds_hat']) + sp.expit(-query['logodds_hat']) * st.gamma(a=np.exp(-query['log_phi_hat']), scale=np.exp(query['log_mu_hat'] + query['log_phi_hat'])).cdf(grid)

     ax[1].plot(grid, G, c=cm(0), label='True g')
     ax[1].plot(grid, Ghat, c=cm(1), label='Estimated $\hat{g}$')
   ...
   :RESULTS:
   # Out[77]:
   [[file:figure/mpebpm.org/sim-ex.png]]
   :END:

   This case appears to be explained by the fact that we initialize at the
   fitted Gamma distribution. It is also outside the range of parameters which
   we previously found ~scqtl~ was able to accurately estimate. Compare the log
   likelihood of the observed data under the two distributions.

   #+BEGIN_SRC ipython
     F = sp.expit(-query['logodds']) * st.nbinom(n=np.exp(-query['log_phi']), p=1 / (1 + 1e5 * np.exp(query['log_mu'] + query['log_phi']))).pmf(x)
     # log(sigmoid(x)) = -softplus(-x)
     l0 = np.where(x == 0, -np.log(1 + np.exp(-query['logodds'])) + np.log(F), np.log(F)).sum()

     Fhat = sp.expit(-query['logodds_hat']) * st.nbinom(n=np.exp(-query['log_phi_hat']), p=1 / (1 + 1e5 * np.exp(query['log_mu_hat'] + query['log_phi_hat']))).pmf(x)
     l1 = np.where(x == 0, -np.log(1 + np.exp(-query['logodds_hat'])) + np.log(Fhat), np.log(Fhat)).sum()
     l0, l1
   #+END_SRC
   ...
   :RESULTS:
   # Out[84]:
   : (-1484.0973317567657, -491.4170443633362)
   :END:
