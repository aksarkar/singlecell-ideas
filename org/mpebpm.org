...
** Application to iPSCs
   ...
   #+BEGIN_SRC ipython :async t
     ...
     np.save('/scratch/midway2/aksarkar/ideas/mpebpm-ipsc-log-mu.npz', log_mu)
     np.save('/scratch/midway2/aksarkar/ideas/mpebpm-ipsc-neg-log-phi.npz', neg_log_phi)
     np.save('/scratch/midway2/aksarkar/ideas/mpebpm-ipsc-logodds.npz', log_mu)
     elapsed / 60
   #+END_SRC
   ...
   #+BEGIN_SRC ipython
     log_mu = np.load('/scratch/midway2/aksarkar/ideas/mpebpm-ipsc-log-mu.npz')
     neg_log_phi = np.load('/scratch/midway2/aksarkar/ideas/mpebpm-ipsc-neg-log-phi.npz')
     log_mu = np.load('/scratch/midway2/aksarkar/ideas/mpebpm-ipsc-logodds.npz')
   #+END_SRC
...
** Application to Census of Immune Cells
   ...
   #+BEGIN_SRC ipython :async t
     start = time.time()
     log_mu, neg_log_phi, _ = mpebpm.ebpm_gamma(y_csr, onehot=onehot, batch_size=32, lr=1e-3, max_epochs=10, verbose=True)
     elapsed = time.time() - start
     elapsed / 60
   #+END_SRC
   ...
   #+BEGIN_SRC ipython
     ...
     pd.DataFrame(-neg_log_phi, index=donor.categories, columns=genes['featurekey']).to_csv('/scratch/midway2/aksarkar/ideas/immune-census-log-phi.txt.gz', sep='\t')
   #+END_SRC
