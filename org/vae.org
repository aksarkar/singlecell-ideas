...
* Model specification and inference
  ...
  \[ x_{ij} \mid \cdot \sim Pois(R_{i} \lambda_{ij}) \]

  \[ \lambda_{ij} \mid \cdot \sim \pi_0(z_{ij}) \delta_0 + (1 - \pi_0)
  g(z_{ij}, u_i) \]
  ...
  #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /scratch/midway2/aksarkar/singlecell/zip-vae.py
    <<imports>>
    <<amsgrad-impl>>
    <<tf-zip-impl>>
