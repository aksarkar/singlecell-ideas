...
* Method
  ...
  2020]]). (Each Gamma distribution is parameterized by shape and rate, and has
  mean \(\mu_{kj}\) and variance \(\mu_{kj}^2\phi_{kj}\).)
  ...
  In the M step, we can improve the likelihood by [[file:mepbpm.org][(batch)
  gradient descent]], implemented in the Python package
  ~mpebpm~. Alternatively, we can use an EM algorithm to solve this subproblem
  (Huang et al. 2019), which introduces auxiliary variables (Adamidis 1999,
  Karlis 2005).

  Another alternative, which is amenable to stochastic gradient descent, is to
  use the fact that EM can be viewed as maximizing the evidence lower bound
