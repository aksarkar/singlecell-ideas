...
** Mechanistic basis of effects on variance
   ...
   \begin{align*}
     x_i \mid \kr, p_i &\sim \operatorname{Poisson}(\kr p_i)\\
     p_i \mid \kon, \koff &\sim \operatorname{Beta}(\kon, \koff)
   \end{align*}

   where:

   - \(x_i\) is the number of molecules in sample \(i\) (considering one gene)
   ...
   and rates are scaled by the decay rate. The Poisson-Beta model model
   describes the steady state distribution of molecules under a two-state
   kinetic model of gene regulation
   ([[https://users.rcc.uchicago.edu/~aksarkar/singlecell-ideas/diff-var.html#poisson-beta][Raj
   et al. 2006]], [[https://dx.doi.org/10.1186/gb-2013-14-1-r7][Kim and Marioni
   2013]], [[https://dx.doi.org/10.1126/science.1216379][Munsky et
   al. 2013]]). Under this model:

   \begin{align*}
     E[x_i] &= \kr \frac{\kon}{\kon + \koff}\\
     V[x_i] &= \kr \frac{\kon}{\kon + \koff} + \kr^2 \frac{\kon\koff}{(\kon + \koff)^2 (\kon + \koff + 1)}\\
     \text{Fano factor} &= 1 + \frac{\kr\koff}{(\kon + \koff) (\kon + \koff + 1)}
   \end{align*}

   From this model, it is clear that an effect which changes variance /while
   leaving the mean unchanged/ must leave \(p_i\) unchanged, i.e. equally scale
   \(\kon\) and \(\koff\). This fact raises several questions:
   ...
   How does this model relate to the typical Poisson-Gamma generative model?

   \begin{align*}
     x_i \mid s_i, \lambda_i &\sim \operatorname{Poisson}(s_i \lambda_i) \\
     \lambda_i \mid \mu, \phi &\sim \operatorname{Gamma}(\phi, \phi / \mu)
   \end{align*}

   where:

   - \(s_i\) denotes the total number of molecules sequenced in sample \(i\)
   ...
   \begin{align*}
     E[x_i] &= s_i \mu \\
     V[x_i] &= s_i \mu + (s_i \mu)^2 \phi \\
     \text{Fano factor} &= 1 + \phi
   \end{align*}

   Suppose data are generated by Poisson-Beta model, and we fit a Poisson-Gamma
   distribution. Naively, it would seem the mean \(\mu\) and overdispersion
   \(\phi\) will be related to each other, through their dependence on \(\kr,
   \kon, \koff\). This possibility raises several questions:
   ...
   The interpretation of the Poisson is different in the two models. In the
   Poisson-Beta model, it denotes stochastic generation of molecules; in the
   Poisson-Gamma model it denotes sampling variation. /How do we account for
   this difference?/ Kim and Marioni assume that the number of molecules
   observed in real data is proportional to the total number of molecules in
   the cell, i.e.

   \[ x_i \sim \operatorname{Poisson}(s_i \kr p_i) \]
...
** Differential variance
   ...
   \begin{align*}
     ...
     p(\lambda_i \mid x_1, \ldots, x_n, s_1, \ldots, s_n, \hat{g}) &= \hat\pi \delta_0(\cdot) + (1 - \hat\pi)\operatorname{Gamma}\left(\cdot; x_i +
