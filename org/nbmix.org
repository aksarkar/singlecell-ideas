...
* Method
  ...
  the Python package ~mpebpm~. the posterior distribution of true
  gene expression given the data is

  \begin{equation}
    \lambda_{ij} \mid x_{ij}, x_{i+}, \vpi, \vmu, \vphi \sim \sum_{k=1}^{K} \pi_{ik} \Gam(x_{ij} + \phi_{kj}^{-1}, x_{i+} + \phi_{kj}^{-1}\mu_{kj}^{-1}).
  \end{equation}

  To make the model more amenable to stochastic gradient descent, we can
  amortize inference by learning a neural network \(f_p\) mapping \(x_{i\cdot}
  \rightarrow z_{i\cdot}\)
