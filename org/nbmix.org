...
* Method
  ...
  assigned to cluster \(k\), the exact posterior \(q \triangleq
  p(z_{ik} \mid \cdot)\) is available

  \begin{align}
    q(z_{i1}, \ldots, z_{iK}) &= \Mult(1, \alpha_{i1}, \ldots, \alpha_{iK})\\
  ...
  The expected log joint with respect to \(q\) is
  ...
  improve the likelihood by [[file:mepbpm.org][(batch) gradient descent]], or
  by mixed analytic and Newton-Raphson updates (Karlis 2005). Alternatively, we
  can write the NB distribution as a Poisson sum of logarithmic series
  distributed random variables (Quenouille 1949), which admits an EM algorithm
  with fully analytic M step
  ([[https://doi.org/10.1111/1467-842X.00075][Adamidis 1999]], Huang et
...
** EM for negative binomial mixture
   :PROPERTIES:
   :CUSTOM_ID: em-nbmix
   :END:
...
** EM for Poisson-Log compound distribution
   ...
   Adamidis 1999 uses the fact that the NB distribution can be derived as a
   Poisson sum of
   [[https://en.wikipedia.org/wiki/Logarithmic_distribution][Log-distributed
   random variables]] (Quenouille 1949)

   \begin{align}
     x_i \mid y_1, \ldots, y_{m_i} &= \sum_{t=1}^{m_i} y_t
     m_i &\sim \Pois(\xiplus \lambda_i)
     p(y_{it} \mid \phi) &= -\frac{\phi^{y_t}}{t \ln(1 - \phi)}, \quad t = 1, \ldots, x_i
   \end{align}

   The key idea is that the parameters of the augmented model admit an analytic
   M step.
