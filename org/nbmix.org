...
** 4-way example
   ...
   #+BEGIN_SRC ipython :async t
     ...
     fit0 = mpebpm.sgd.ebpm_gamma(
       mix4sub.X,
       onehot=pd.get_dummies(mix4sub.obs['cell_type']).values,
       batch_size=32,
       num_epochs=num_epochs,
       shuffle=True,
   ...
   Report the minimum cluster weight, over MAP estimates for each cell.

   #+BEGIN_SRC ipython
     np.max(fit[-1], axis=1).min()
   #+END_SRC
   ...
   Compare the expected log joint of the fitted model to the oracle.
