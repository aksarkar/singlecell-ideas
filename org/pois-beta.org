...
** Slice sampling
   ...
   \begin{align*}
     ...
     \ln p(\kon \mid \cdot) &= \sum_i [\kon \ln p_i + (\aon - 1)\ln \kon - \bon\kon - 2 \ln\Gamma(\kon) + \ln\Gamma(\kon + \koff)] + \const\\
     \ln p(\koff \mid \cdot) &= \sum_i [\koff \ln (1 - p_i) + (\aoff - 1)\ln \koff - \boff\koff - 2 \ln\Gamma(\koff) + \ln\Gamma(\koff + \koff)] + \const
   \end{align*}
