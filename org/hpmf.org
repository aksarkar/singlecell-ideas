...
** Simulated example
   ...
   Fit the model via VBEM. To simplify, fix the hyperparameters at the oracle
   values.

   #+BEGIN_SRC ipython :async t
     # Important: we randomly initialized
     torch.manual_seed(1)
     m1 = PMFVBEM(n, p, K).cuda().fit(xt, num_epochs=1000)
   #+END_SRC
   ...
   # Out[17]:
   #+BEGIN_EXAMPLE
     array([[ 0.01895363, -0.07767732,  0.99240795],
     [ 0.99418499,  0.03297424,  0.02721594],
     [ 0.02447772,  0.99437128, -0.05625653]])
   #+END_EXAMPLE
   :END:
   ...
   Fit the model via pathwise gradient, initializing at the VBEM solution. To
   make the comparison more fair, continue to fix the hyperparameters at the
   oracle value.

   #+BEGIN_SRC ipython :async t
     # Important: this algorithm is stochastic
     torch.manual_seed(2)
     m2 = PMF(n, p, K).cuda()
     with torch.no_grad():
       m2.alpha_l.data = m1.alpha_l.clone()
       m2.beta_l.data = m1.beta_l.clone()
       m2.alpha_f.data = m1.alpha_f.clone()
       m2.beta_f.data = m1.beta_f.clone()
   ...
   # Out[87]:
   #+BEGIN_EXAMPLE
     array([[ 0.01809326, -0.07844437,  0.9924126 ],
     [ 0.99418027,  0.03239465,  0.02689676],
     [ 0.02408282,  0.9943834 , -0.05662122]])
   #+END_EXAMPLE
   :END:
   ...
   #+BEGIN_SRC ipython :ipyfile figure/hpmf.org/sim-elbo.png
     ...
     plt.yscale('log')
     plt.plot(pd.Series(m1.trace).ewm(alpha=0.1).mean(), c=cm(0), lw=1, marker=None, label='VBEM')
     plt.plot(np.arange(1001, 5001), pd.Series(m2.trace).ewm(alpha=0.1).mean(), c=cm(1), lw=1, marker=None, label='Grad')
   ...
   # Out[90]:
   #+BEGIN_EXAMPLE
     {'alpha_l': array(43.566395, dtype=float32),
     'beta_l': array(3.7134445, dtype=float32),
     'alpha_f': array(54.408215, dtype=float32),
     'beta_f': array(3.3419561, dtype=float32)}
   #+END_EXAMPLE
   :END:
