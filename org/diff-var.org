...
*** Variational inference
   ...
   \begin{multline}
     \ell = \E_{q(\mu, \phi, \delta)}\left[\sum_i \ln p(x_i \mid s_i, z_i, \mu, \phi, \delta)\right]\\
...
*** MCMC
    ...
    \begin{equation}
      \ln p(x_i, \mu, \phi, \delmu, \delphi \mid s_i, z_i) = x_i \ln \left(\frac{s_i}{s_i + b_i}\right) + a_i \ln \left(\frac{b_i}{s_i + b_i}\right) + \ln\Gamma(x_i + a_i) - \ln\Gamma(a_i) - \ln\Gamma(x_i + 1) - \frac{(\ln\mu - \ln\mu_0)^2}{2\mu} - \frac{(\ln\phi - \ln\phi_0)^2}{2\phi},
    \end{equation}
...
*** Null example
   ...
   #+BEGIN_SRC ipython :ipyfile figure/diff-var.org/mcmc-vs-vi-ex0.png
     plt.clf()
     fig, ax = plt.subplots(1, 4)
     fig.set_size_inches(7.5, 2.5)
     for i, a in enumerate(ax):
   ...
   :RESULTS:
   # Out[57]:
   [[file:figure/diff-var.org/mcmc-vs-vi-ex0.png]]
   :END:
