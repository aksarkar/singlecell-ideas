...
** Bayesian approaches to detect differential variance
   ...
   form informative priors on \(\vtheta = (\mu, \phi, \delmu, \delphi)\).
...
*** Variational inference
   ...
   \(q(\delphi) \approx p(\delphi \mid \vx, \vs)\), assuming 

   \begin{equation}
     q(\vtheta) = q(\mu)q(\phi)q(\delmu)q(\delphi)
   \end{equation}
   ...
   where the KL terms and expectations with respect to \(q(\lambda_i)\) are all
   analytic, and the first (outer) expectation is computed using a Monte Carlo
   integral and a differentiable re-parameterization (Kingma and Welling 2014).
...
*** MCMC                                                           :noexport:
    ...
    \begin{equation}
      \ln p(x_i, a_i, b_i, \delmu, \delphi \mid s_i, z_i) = x_i \ln \left(\frac{s_i}{s_i + b_i}\right) + a_i \ln \left(\frac{b_i}{s_i + b_i}\right) + \ln\Gamma(x_i + a_i) - \ln\Gamma(a_i) - \ln\Gamma(x_i + 1) - \frac{(\ln\mu - \ln\mu_0)^2}{2\mu} - \frac{(\ln\phi - \ln\phi_0)^2}{2\phi},
    \end{equation}

    and the log conditional probabilities (up to a constant) are

    \begin{align}
      \ln p(\mu \mid \cdot) &= \sum_i u_i - (\ln \mu - \ln\mu_0)^2 / (2 \mu) + \const\\
      \ln p(\phi \mid \cdot) &= \sum_i u_i + v_i - (\ln \phi - \ln\phi_0)^2 / (200 \phi) + \const\\
...
*** DE
   ...
   #+BEGIN_SRC ipython :ipyfile figure/diff-var.org/vi-ex1.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(4, 2)
     grid = np.arange(x.max() + 2)
   ...
   :RESULTS:
   # Out[48]:
   [[file:figure/diff-var.org/vi-ex1.png]]
   :END:
   ...
   #+BEGIN_SRC ipython :async t
     ...
     m.fit(
       ...
       n_epochs=n_epochs,
       n_samples=n_samples,
       log_dir=f'/scratch/midway2/aksarkar/singlecell/runs/diffvar/dev_2fold_{n_epochs}_{n_samples}_{run}')
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/diff-var.org/vi-ex1-fit.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(4, 2)
     grid = np.arange(x.max() + 2)
   ...
   :RESULTS:
   # Out[78]:
   [[file:figure/diff-var.org/vi-ex1-fit.png]]
   :END:
...
*** DD
   ...
   #+BEGIN_SRC ipython :ipyfile figure/diff-var.org/vi-ex2.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(4, 2)
     grid = np.arange(x.max() + 2)
   ...
   :RESULTS:
   # Out[97]:
   [[file:figure/diff-var.org/vi-ex2.png]]
   :END:
   ...
   #+BEGIN_SRC ipython :async t
     ...
     m.fit(
       ...
       n_epochs=n_epochs,
       n_samples=n_samples,
       log_dir=f'/scratch/midway2/aksarkar/singlecell/runs/diffvar/dev_dd_2fold_{n_epochs}_{n_samples}_{run}')
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/diff-var.org/vi-ex2-fit.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(4, 2)
     grid = np.arange(x.max() + 2)
   ...
   :RESULTS:
   # Out[123]:
   [[file:figure/diff-var.org/vi-ex2-fit.png]]
   :END:
