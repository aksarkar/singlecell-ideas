...
** Application to Census of Immune Cells
   ...
   #+BEGIN_SRC ipython :results output html verbatim
     ...
     with loompy.connect('/project2/mstephens/aksarkar/projects/singlecell-ideas/data/human-cell-atlas/immune-cell-census.loom') as con:
       print(pd.DataFrame(np.hstack([con.ca['derived_organ_parts_label','donor_organism.human_specific.ethnicity.ontology_label'], con.ca['donor_organism.provenance.document_id', 'donor_organism.sex',]])).groupby([0,3,1,2]).agg(len).to_frame().to_html(classes='table'))
   #+END_SRC
   ...
   To demonstrate the scalability of ~mpebpm~, fit a Gamma distribution to each
   gene in each donor. We
