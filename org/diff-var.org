...
*** iPSC-CM time course data
    ...
    #+BEGIN_SRC ipython :ipyfile figure/diff-var.org/ipsc-cm-log-phi.png
      ...
      for k, z in res.loc(axis=1)[:,'neg_log_phi'].iterrows():
        if z.mean() > -10:
          plt.plot(z.values.ravel(), c='k', lw=1, alpha=0.25)
      plt.xlabel('Day')
    ...
    #+BEGIN_SRC ipython :async t
      ...
      for k in [0, 1, 3, 7]:
        ...
        query = y.X[:,idx].A.ravel()
        samples, trace = poisbeta.fit_poisson_beta_mcmc(query, n_samples=1000, ar=1, br=query.max(), aon=1, bon=100, aoff=1, boff=100)
        k_params[k] = samples
        traces[k] = trace
      with open('/scratch/midway2/aksarkar/ideas/ipsc-cm-mcmc.pkl', 'wb') as f:
        pickle.save((k_params, traces), f)
    #+END_SRC
