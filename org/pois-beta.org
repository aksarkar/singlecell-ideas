...
** Impact of size factors
   ...
   Simulate a simple example with non-trivial size factor variation.
   ...
   Plot the estimated distributions, as well as the difference between the
   fitted Beta distributions and the ground truth.
   ...
   #+BEGIN_SRC ipython :ipyfile figure/pois-beta.org/size-ex.png
     ...
     for i, (log_theta, label) in enumerate(zip(res, ['Ground truth', 'Moment', 'MLE (posthoc)', 'MLE'])):
       p = np.array([np.exp(poisson_beta_logpmf(log_theta, i, s)).mean() for i in grid[:-1]])
