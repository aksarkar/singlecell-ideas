...
** Normalizing flow for empirical Bayes
     ...
     class EBNM(torch.nn.Module):
       ...
       def forward(self, x, n_samples):
         q0 = torch.distributions.Normal(
           ...
           scale=torch.sqrt(1 / (1 + self.scale ** 2)))
         u = q0.rsample([1]).squeeze(-1)
         z, log_det_q = self.qz.forward(u.T)
   ...
   #+BEGIN_SRC ipython
     class EBPM(torch.nn.Module):
       def __init__(self, K):
         super().__init__()
         self.pz = NormalizingFlow([PlanarFlow(1) for _ in range(K)])
         self.qz = NormalizingFlow([PlanarFlow(1) for _ in range(K)])

       def forward(self, x, s, n_samples):
         u = N.rsample([x.shape[0], 1])
         log_lam, log_det_q = self.qz.forward(u)
         lam = torch.exp(log_lam)
         log_det_q += log_lam
         w, log_det_p = self.pz.forward(lam)
         # Important: qz is forward transforms, so we need to invert the sign of
         # log_det_q
         kl = N.log_prob(w) + log_det_p - (N.log_prob(u) - log_det_q)
         assert (kl > 0).all()
         elbo = (torch.distributions.Poisson(s * lam).log_prob(x) - kl).sum()
         assert elbo <= 0
         return -elbo
       ...
       def fitted_g(self, lam):
         u, log_det = self.pz.forward(lam)
         return N.log_prob(u) + log_det
...
** EBNM via flows sanity check
   ...
   Plot the approximate posterior mean against the true posterior mean.

   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-q.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(2.5, 2.5)
     q0 = st.norm(x.reshape(-1, 1) / (1 + s ** 2), np.sqrt(1 / (1 + 1 / s ** 2)))
     for k, m in enumerate(models):
       with torch.no_grad():
         samples = np.stack([m.qz.forward(torch.tensor(q0.rvs(), dtype=torch.float))[0].numpy() for _ in range(100)]).squeeze()
         muhat = samples.mean(axis=0)
       plt.scatter(q0.mean().ravel(), muhat, s=1, c=np.array(cm(k)).reshape(1, -1), label=f'K = {k+1}')
     l = plt.legend(frameon=False, handletextpad=0, markerscale=4)
     lim = [mu.min(), mu.max()]
     plt.plot(lim, lim, lw=1, ls=':', c='r')
     plt.xlim(lim)
     plt.ylim(lim)
     plt.xlabel('Exact posterior mean')
   ...
   :RESULTS:
   # Out[43]:
   [[file:figure/flows.org/ebnm-q.png]]
   :END:

   Compare samples from the approximate posterior, holding \(K=2\), for the
   largest observation against the true posterior.

   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-q-max.png
     plt.clf()
     plt.gcf().set_size_inches(2.5, 2.5)
     idx = np.argmax(mu)
     q0 = st.norm(x[idx] / (1 + s ** 2), np.sqrt(1 / (1 + 1 / s ** 2)))
     u = q0.rvs(size=(500, 1))
     with torch.no_grad():
       samples = models[1].qz.forward(torch.tensor(u, dtype=torch.float))[0].numpy()
     plt.hist(samples, bins=3, density=True, color='0.8')
     grid = np.linspace(2, 4, 1000)
     plt.plot(grid, q0.pdf(grid), lw=2, c='k')
     plt.xlabel('Latent variable $\mu$')
   ...
   :RESULTS:
   # Out[56]:
   [[file:figure/flows.org/ebnm-q-max.png]]
   :END:

   Look at different random initializations for \(K=4\).
   ...
   #+BEGIN_SRC ipython :async t
     ...
     for run in range(5):
       torch.manual_seed(run)
       models.append(EBNM(K=4, scale=torch.tensor(s))
                     .fit(torch.tensor(x.reshape(-1, 1), dtype=torch.float),
                          n_epochs=n_epochs,
                          lr=lr))
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-k=4-init.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     fig, ax = plt.subplots(2, 1)
     fig.set_size_inches(4, 4)
     ...
     ax[1].plot(grid, st.norm().pdf(grid), lw=2, c='k', label='Simulated')
     for k, m in enumerate(models):
       ax[1].plot(grid, m.fitted_g(torch.tensor(grid.reshape(-1, 1), dtype=torch.float), log=False), c=cm(k), label=f'Run = {k + 1}')
     ax[1].legend(frameon=False, loc='center left', bbox_to_anchor=(1, .5))
     ax[1].set_xlabel('Latent variable $\mu$')
     ax[1].set_ylabel('Density')
     fig.tight_layout()
   ...
   :RESULTS:
   # Out[34]:
   [[file:figure/flows.org/ebnm-k=4-init.png]]
   :END:

   Look at the zero initialization.
   ...
   #+BEGIN_SRC ipython :async t
     ...
     n_epochs = 20000
     torch.manual_seed(run)
     models = [EBNM(K=K, scale=torch.tensor(s), random_init=False)
               .fit(torch.tensor(x.reshape(-1, 1), dtype=torch.float),
                    n_epochs=n_epochs,
                    lr=lr)
               for K in range(1, 5)]
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-zero-init.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     fig, ax = plt.subplots(2, 1)
     fig.set_size_inches(4, 4)
   ...
   :RESULTS:
   # Out[13]:
   [[file:figure/flows.org/ebnm-zero-init.png]]
   :END:

   Compute the analytic ELBO (where \(q\) equals the true posterior).

   #+BEGIN_SRC ipython
     q0 = st.norm(x / (1 + s ** 2), np.sqrt(1 / (1 + 1 / s ** 2)))
     elbo0 = -.5 * (np.log(2 * np.pi * s ** 2) +
                    (np.linalg.norm(x - q0.mean()) - q0.var()) / s ** 2 +
                    (1 - 2 * np.log(q0.var()) + (q0.mean() ** 2 + q0.var()))).sum()
     elbo0
   #+END_SRC
   ...
   Look at the stochastic estimate of the ELBO for each model.

   #+BEGIN_SRC ipython
     torch.manual_seed(100)
     with torch.no_grad():
       elbos = pd.Series({
         k + 1: np.stack([-m.forward(torch.tensor(x.reshape(-1, 1), dtype=torch.float), [1]).numpy()
...
** Example of Empirical Bayes Poisson Means
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ex1.png
     plt.clf()
     fig, ax = plt.subplots(2, 1)
     fig.set_size_inches(4, 4)
     grid = np.arange(x.max() + 1)
     ax[0].hist(x, bins=grid, color='0.7', density=True)
     ax[0].plot(grid + 0.5, st.nbinom(n=np.exp(log_inv_disp), p=1 / (1 + s[0] * np.exp(log_mean - log_inv_disp))).pmf(grid), lw=1, c='k')
     ax[0].set_xlabel('Observed count')
     ax[0].set_ylabel('Density')
     grid = np.linspace(0, (x / s).max(), 1000)
     ax[1].plot(grid, st.gamma(a=np.exp(log_inv_disp), scale=np.exp(log_mean - log_inv_disp)).pdf(grid), lw=1, c='k')
   ...
   #+BEGIN_SRC ipython :async t
     ...
     m = (EBPM(K=K)
          .fit(torch.tensor(x.reshape(-1, 1), dtype=torch.float),
               ...
               n_epochs=n_epochs,
               lr=lr,
               log_dir=f'/scratch/midway2/aksarkar/singlecell/runs/ebpm-nf-{K}-{lr}-{n_epochs}-{run}'))
   #+END_SRC
