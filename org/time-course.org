...
** Simulation
   ...
   #+BEGIN_SRC ipython
     with pickle.open('/scratch/midway2/aksarkar/ideas/anmf-sim.pkl', 'rb') as _ff:
   ...
   :RESULTS:
   # Out[14]:
   : (0.02684265002465202, 0.02111822607549802)
   :END:
   ...
   #+BEGIN_SRC ipython :async t
     ...
     fit = (anmf.modules.ANMF(input_dim=x.shape[1], latent_dim=k)
            .fit(data, max_epochs=1000, trace=True, lr=1e-2))
     elapsed = time.time() - start
     elapsed
   #+END_SRC
   ...
   :RESULTS:
   # Out[75]:
   : (0.08683096761321933, 0.08068438603560046)
   :END:
