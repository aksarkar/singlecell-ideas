...
** Example of EBPM
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebpm-gamma-ex.png
     ...
     for i, k in enumerate(models):
       F = np.array([scipy.integrate.simps(
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebpm-gamma-ex-q.png
     ...
     for i, a in enumerate(ax):
       grid = np.log(np.linspace(1e-4, 5e-3, 1000))
       q = st.gamma(a=np.exp(log_inv_disp) + x_grid[i], scale=1 / (np.exp(log_mean - log_inv_disp) + s[0]))
       ax[i].plot(grid, q.pdf(np.exp(grid)) * np.exp(grid), lw=2, c='k', label='$p_{\mathrm{post}}$')
       q0 = st.gamma(a=1 + x_grid[i], scale=1 / (1 + s[0]))
