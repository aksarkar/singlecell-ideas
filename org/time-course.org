...
** Amortized NMF/LDA
   ...
   variable. These can be optimized in alternating phases.) The optimization
   can be achieved by introducing an /encoder network/ \(h_\alpha\), which has
   been previously proposed ([[doi:10.1038/s41592-018-0229-2][Lopez et
   al. 2018]], [[doi:10.1038/s41467-018-07931-2][Eraslan et al. 2018]]).
   ...
   implementation based on minibatch gradient descent, which we provide in the
   Python package ~anmf~.
...
** Simulation
   ...
   #+BEGIN_SRC ipython
     ...
     data = td.DataLoader(sparse_data, batch_size=512, shuffle=False, collate_fn=sparse_data.collate_fn)
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :async t
     fit = anmf.modules.ANMF(input_dim=x.shape[1]).fit(data, max_epochs=1, trace=True, verbose=True, lr=5e-3)
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :async t
     lhat = np.vstack([fit.encoder.forward(b).detach().numpy() for (b, s) in data])
     fhat = torch.nn.functional.softmax(fit.decoder.logit_f, dim=1).detach().numpy()
     A = x / (s.reshape(-1, 1) * lhat.dot(fhat))
     l_resid = (fhat.T * ((1 - A).T @ lhat)).max()
     f_resid = (lhat * ((1 - A) @ fhat.T)).max()
     l_resid, f_resid
   #+END_SRC
   ...
   :RESULTS:
   # Out[90]:
   : (0.010334900542034623, -0.0)
   :END:

** ANMF on Census of Immune Cells
