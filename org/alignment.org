...
* Setup                                                            :noexport:
  ...
  #+CALL: ipython3(memory="32G",venv="singlecell",partition="gpu2",opts="--gres=gpu:1") :dir /scratch/midway2/aksarkar/ideas

  #+RESULTS:
  : Submitted batch job 51849776
...
** ZIPAAE on known mixtures
   ...
   #+BEGIN_SRC ipython
     ...
     batch = (np.random.uniform(size=mix.shape[0]) < 0.5).astype(int)
   #+END_SRC
...
** Comparison to existing methods
   ...
   #+BEGIN_SRC ipython
     mix = ss.hstack([control, stim]).tocsr()
     mix = mix[((mix > 0).mean(axis=1) >= 0.25).A.ravel()].T.astype(np.int)
     mix.shape
   #+END_SRC
   ...
   #+BEGIN_SRC ipython
     qz = align(mix, y, latent_dim=10)
   #+END_SRC


   #+BEGIN_SRC ipython
     def align(counts, labels, latent_dim, batch_size=100, max_epochs=2, verbose=False):
       ...
       # Fit the model
       with torch.cuda.device(0):
         model = scaa.modules.ZIPAAE(input_dim=counts.shape[1], latent_dim=latent_dim, num_classes=2).fit(x=training_data, y=batch_data, max_epochs=max_epochs, verbose=verbose)
