...
* Setup                                                            :noexport:
  ...
  #+CALL: ipython3(memory="4G",partition="gpu2",venv="singlecell",opts="--gres=gpu:1") :dir /scratch/midway2/aksarkar/ideas

  #+RESULTS:
  : Submitted batch job 51480571
...
** ZIPVAE on known mixture
   ...
   Embed a known mixtures of sorted cells, then look at whether they are
...
** ZIPAAE sanity check
   ...
   Use real sorted cells, and repeat the benchmark.

   #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-ideas/code/zipaae-cd8.py
     <<imports>>
     ...
     def evaluate(data, num_trials):
       ...
       for trial in range(num_trials):
         ...
         y = (np.random.uniform(size=x.shape[0]) < 0.5).astype(np.int)
         result.append([trial,
                        scaa.benchmark.generalization_score_plra1(train, test),
                        scaa.benchmark.generalization_score_zipvae(train, test),
                        scaa.benchmark.generalization_score_zipaae(train, test, y=y),
         ])
       result = pd.DataFrame(result)
       result.columns = ['trial', 'PLRA1', 'ZIPVAE', 'ZIPAAE']
       return result

     res = evaluate(cd8_cytotoxic_t_cells, num_trials=1)
     # res.to_csv('/project2/mstephens/aksarkar/projects/singlecell-ideas/data/alignment/zipaae-generalization2.txt.gz', compression='gzip', sep='\t')
   #+END_SRC
   ...
   #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/ideas/
     ...
     #!/bin/bash
     source activate singlecell
     python /project2/mstephens/aksarkar/projects/singlecell-ideas/code/zipaae-cd8.py
   #+END_SRC

   #+RESULTS:
   : Submitted batch job 51508003

** ZIPAAE on known mixtures
** Comparison to existing methods

   Look at control versus stimulated CD4+ T cells from
   [[https://www.nature.com/articles/nbt.4042][Kang et al 2018]].
   ...
   #+BEGIN_SRC ipython
     control = scipy.io.mmread("/scratch/midway2/aksarkar/ideas/GSE96583/GSM2560248_2.1.mtx.gz")
     stim = scipy.io.mmread("/scratch/midway2/aksarkar/ideas/GSE96583/GSM2560248_2.2.mtx.gz")
   #+END_SRC
