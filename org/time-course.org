...
** Simulation
   ...
   #+BEGIN_SRC ipython :async t
     A = x / (s.reshape(-1, 1) * lhat0 @ fhat0.T)
   ...
   :RESULTS:
   # Out[53]:
   : (2567.8499292412193, 7533.948618377116)
   :END:
   ...
   #+BEGIN_SRC ipython :async t
     ...
     fit = (anmf.modules.ANMF(input_dim=x.shape[1])
            .fit(data, max_epochs=800, trace=True, lr=5e-4))
     elapsed = time.time() - start
     elapsed
   #+END_SRC
   ...
   \(\vl_i\) for each observation \(\vx_i\). The more important (and more
   difficult!) question to assess is whether this difference matters for
   downstream analysis.
