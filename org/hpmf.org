...
** Simulated example
   ...
   #+BEGIN_SRC ipython :async t
     ...
     m2.fit(xt, num_epochs=10000, lr=5e-2)
   ...
   :RESULTS:
   # Out[255]:
   : {'VBEM': 105008.25, 'GD': 105009.27, 'Newton': 104985.46}
   :END:
...
** Correlated factors example
   ...
   #+BEGIN_SRC ipython :ipyfile figure/hpmf.org/corr-sim-elbo.png
     ...
     for i, (model, name) in enumerate(zip((m1, m2, m3), ('VBEM', 'Grad', 'Newton'))):
       ...
       ax[0].fill_between(np.arange(len(mu)), mu - sd, mu + sd, color=cm(i), alpha=0.1)
     for i, (model, name) in enumerate(zip((m1, m2, m3), ('VBEM', 'Grad', 'Newton'))):
       mu = pd.Series(model.trace).ewm(alpha=0.05).mean()
       sd = pd.Series(model.trace).ewm(alpha=0.05).std()
       ax[1].plot(mu, c=cm(i), lw=1, marker=None, label=name)
       ax[1].fill_between(np.arange(len(mu)), mu - sd, mu + sd, color=cm(i), alpha=0.1)
     ax[1].set_xlim(0, 200)
     ax[1].legend(frameon=False, loc='center left', bbox_to_anchor=(1, .5))
   ...
   :RESULTS:
   # Out[275]:
   : {'VBEM': 118743.36, 'GD': 118731.02, 'Newton': 118729.98}
   :END:
