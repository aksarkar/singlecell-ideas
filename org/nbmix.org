...
** 10-way example
   ...
   #+BEGIN_SRC ipython :ipyfile figure/nbmix.org/mix10-full-std.png
     ...
     for i, c in enumerate(dat.obs['leiden'].unique()):
       ax[1].plot(*dat[dat.obs['leiden'] == c].obsm["X_umap"].T, c=cm(i), marker='.', ms=1, lw=0, label=f'Cluster {i}', alpha=0.1)
   ...
   First, fit a point mass expression model, given the ground truth labels.
