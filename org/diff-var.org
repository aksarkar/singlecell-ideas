...
** Differential variance
   ...
   As a third alternative, we could re-parameterize the model in terms of
   differential dispersion, and perform posterior inference on the
   difference. Fixing gene \(j\),
   ...
   #+BEGIN_SRC ipython
     ...
     def _cond_logpdf_delta(delta, lam, z, mu, phi, sigma0):
       t = (phi + z * delta)
       return (z * delta * _safe_log(t) + z * delta * _safe_log(lam) - mu * z * delta * lam - sp.gammaln(t) - _safe_log(delta)^2 / (2 * delta * sigma0 * sigma0)).sum()

     def diff_var_mcmc(x, s, z, n_samples, sigma0=10, verbose=False, trace=False):
       assert sigma0 > 0
       assert n_samples > 0
       Fmu = st.lognorm(scale=sigma0)
       Fphi = st.lognorm(scale=sigma0)
       Fdelta = st.lognorm(scale=sigma0)
       ...
       for t in range(n_samples):
         ...
         # p(lam | .) is analytic
         t = phi + z * delta
         lam = st.gamma(a=x + t, scale=mu * t).rvs()
         ...
         delta = slice_sample(_cond_logpdf_delta, init=delta, lam=lam, z=z, mu=mu, delta=delta, sigma0=sigma0, bounds=[1e-8, np.inf])
         if trace or verbose:
           log_joint = (st.poisson(mu=s * lam).logpmf(x).sum()
                        + st.gamma(a=x + t, scale=mu * t).logpdf(lam).sum()
                        + Fmu.logpdf(mu)
                        + Fphi.logpdf(phi)
                        + Fdelta.lgpdf(delta))
           if trace:
