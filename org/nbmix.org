...
** 4-way example
   ...
   Pick 4 cell types which /a priori/ should be easy to distinguish. Apply the
   ...
   #+BEGIN_SRC ipython :ipyfile figure/nbmix.org/mix4-cd74.png
   ...
   :RESULTS:
   # Out[115]:
   [[file:figure/nbmix.org/mix4-cd74.png]]
   :END:
   ...
   #+BEGIN_SRC ipython :ipyfile figure/nbmix.org/mix4-nmf.png
   ...
   :RESULTS:
   # Out[50]:
   [[file:figure/nbmix.org/mix4-nmf.png]]
   :END:
   ...
   #+BEGIN_SRC ipython
     ...
     idx = [np.argmax(weights[z[:,k]].sum(axis=0)) for k in range(4)]
   ...
   #+BEGIN_SRC ipython
     (np.argmax(weights[:,idx], axis=1) == np.argmax(z, axis=1)).mean()
   #+END_SRC
   ...
   #+BEGIN_SRC ipython
     z = pd.get_dummies(mix4sub.obs['cell_type']).values.astype(bool)
     idx = [np.argmax(fit_init_z[-1][z[:,k]].sum(axis=0)) for k in range(4)]
   ...
   #+BEGIN_SRC ipython
     (np.argmax(fit_init_z[-1][:,idx], axis=1) == np.argmax(z, axis=1)).mean()
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/nbmix.org/mix4-init-z.png
     plt.clf()
     fig, ax = plt.subplots(1, 2, sharex=True, sharey=True)
     plt.gcf().set_size_inches(5, 3.5)
   ...
   :RESULTS:
   # Out[53]:
   [[file:figure/nbmix.org/mix4-init-z.png]]
   :END:
   ...
   #+BEGIN_SRC ipython :ipyfile figure/nbmix.org/mix4-init-mu.png
     plt.clf()
     fig, ax = plt.subplots(1, 2, sharex=True, sharey=True)
     plt.gcf().set_size_inches(5, 3.5)
   ...
   :RESULTS:
   # Out[56]:
   [[file:figure/nbmix.org/mix4-init-mu.png]]
   :END:
