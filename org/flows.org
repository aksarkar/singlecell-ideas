...
** Planar flow
   ...
   where \(u, w, b\) are free (scalar) parameters. The intuition behind this
   transform is that the pre-activation \(w x + b\) defines a (hyper)plane, and
   the post-activation \(u\) dilates the density about that hyperplane.
...
** Density estimation sanity check
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ex2.png
     ...
     plt.plot(grid, st.norm().pdf(grid), lw=1, c='k', label='Simulated')
...
** EBNM via flows sanity check
   ...
   #+BEGIN_SRC ipython
     ...
     scale = np.array([1., 1.])
     z = rng.uniform(size=(n, 1)) < pi[0]
     mu = rng.normal(scale=scale @ np.hstack([z, ~z]).T)
     x = rng.normal(loc=mu, scale=s)
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-ex.png
     ...
     ax[1].plot(grid[:-1], np.diff(mixcdf) / (grid[1] - grid[0]), lw=1, c='k')
   ...
   #+BEGIN_SRC ipython :async t
     ...
     models = [(EBNM(K=K, scale=torch.tensor(s))
                .fit(torch.tensor(x.reshape(-1, 1), dtype=torch.float),
                     n_epochs=n_epochs,
                     lr=lr,
                     log_dir=f'/scratch/midway2/aksarkar/singlecell/runs/ebnm-nf-{K}-{lr}-{n_epochs}-{run}'))
               for K in range(1, 5)]
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-fitted-g.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(3.5, 2.5)
     plt.plot(grid, st.norm(scale=scale).pdf(grid.reshape(-1, 1)) @ pi, lw=1, c='k', label='Simulated')
     grid = np.linspace(mu.min(), mu.max(), 1000)
     for k, m in enumerate(models):
       plt.plot(grid, m.fitted_g(torch.tensor(grid, dtype=torch.float).reshape(-1, 1), log=False), lw=1, c=cm(k), label=f'K={k+1}')
     plt.legend(frameon=False, loc='center left', bbox_to_anchor=(1, .5))
   ...
   :RESULTS:
   # Out[45]:
   [[file:figure/flows.org/ebnm-fitted-g.png]]
   :END:

   Plot the approximate posterior mean against the true values of the latent
   variables.
   ...
   #+BEGIN_SRC ipython :ipyfile figure/flows.org/ebnm-q.png
     ...
     for k, m in enumerate(models):
       ...
       plt.scatter(mu, muhat, s=1, c=np.array(cm(k)).reshape(1, -1), label=f'K={k+1}')
