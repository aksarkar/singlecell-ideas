...
** Amortized NMF
   ...
   \[ \theta_i^* = \argmin_{\theta_i} f(\theta_i, \phi_i) \]

   where \(\phi_i \in \Phi\) is some context variable. Rather than solving one
   problem for each \(\phi_i\), we learn a function \(h_\alpha\) parameterized
   by \(\alpha\) mapping \(\phi_i \rightarrow \theta_i^*\)
   ...
   al. 2018]], [[doi:10.1038/s41467-018-07931-2][Eraslan et
   al. 2018]]). 

   However, existing methods have a major limitation: they do not support
   sparse matrices on the GPU, making it either impossible to analyze complete
   data sets, or introducing a major bottleneck in moving minibatches to the
   GPU. We implement this functionality in the Python package ~anmf~.
