...
** 10-way mixture
   ...
   #+BEGIN_SRC ipython :async t
     ...
     sc.pp.filter_genes(x, min_cells=.001 * x.shape[0])
   #+END_SRC
...
** ANMF on 10-way mixture
   ...
   #+BEGIN_SRC ipython :async t
     start = time.time()
     fit = anmf.modules.ANMF(input_dim=x.shape[1]).fit(data, max_epochs=10, trace=True, verbose=True, lr=1e-2)
     elapsed = time.time() - start
   #+END_SRC
