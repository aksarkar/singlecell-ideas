...
** Differential variance
   ...
   \(p\)-values. There are two challenges in this approach:
   ...
   #+BEGIN_SRC ipython
     ...
     def zinb_variance_bootstrap(x, s, z, random_state=None, n_bootstrap=1000):
...
** Null simulation
     ...
     def evaluate_null_type1(data, n_trials):
       ...
       for trial in range(n_trials):
         ...
         for gene, counts in x.iteritems():
           try:
             llr, p = zinb_variance_lrt(counts.values, s, z)
           except RuntimeError:
             llr, p = np.nan, np.nan
   ...
   #+BEGIN_SRC ipython :async t
     evaluate_null_type1(cytotoxic_t, n_trials=1)
   #+END_SRC
